...
#define MAX_NO_RX_INTERVAL                                                      \
  60000 * 2 * 65 // This should be high then one adverrtising interval to prevent false positives.
#endif
...
bool Dispatcher::tryParsePacket(Packet *pkt, const uint8_t *raw, int len) {
  ...
  memcpy(pkt->path, &raw[i], path_byte_len);
  i += path_byte_len;

  pkt->payload_len = len - i; // payload is remainder
  if (pkt->payload_len > sizeof(pkt->payload)) {
    MESH_DEBUG_PRINTLN("%s Dispatcher::checkRecv(): packet payload too big, payload_len=%d", getLogDateTime(),
                       (uint32_t)pkt->payload_len);
    return false;
  }
